...
class MaterialCalendarPickerDemoComponent {
  ...
  CalendarState plainModel = CalendarState.selected([
    CalendarSelection('range', date(-4), date(4)),
  ]);
  ...
  CalendarState mutableModel = CalendarState.selected([
    CalendarSelection('range', date(0), date(4)),
  ]);

  void creepForward() {
    var current = mutableModel.selection('range');
    mutableModel = mutableModel.setSelection(
      CalendarSelection(
        'range',
        current.start!.add(days: 1),
        current.end!.add(days: 1),
      ),
    );
  }

  CalendarState singleDateModel = CalendarState.selected([
    CalendarSelection('range', date(0), date(0)),
  ]);

  CalendarState dateRangeModel = CalendarState.selected([
    CalendarSelection('range', date(-7), date(0)),
  ]);
}
