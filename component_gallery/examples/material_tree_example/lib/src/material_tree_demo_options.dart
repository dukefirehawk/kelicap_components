...
final SelectionOptions simpleFlatOptions = SelectionOptions.fromList(
  _commonOptionList,
);

/// An example data set of non-hierarchical data that is [Filterable].
final SelectionOptions filterableFlatOptions = StringSelectionOptions(
  _commonOptionList,
);
...
class _NestedFilterableSelectionOptions<T extends String>
    extends _NestedSelectionOptions<T>
    with Filterable {
  @override
  int currentLimit = -1;
  ...
  _NestedFilterableSelectionOptions(
    List<OptionGroup<T>> options,
    Map<T, List<OptionGroup<T>>> children,
...
NestedCategoryOptions expandStateOptions() => NestedCategoryOptions([
  OptionGroup([
    ...
    _ctn('Animated', [
      _ctn('2D Animated', [
        _ctn('Childrens\' Animation'),
        _ctn('Adults\' Animation'),
      ]),
