...
class ComparisonOption {
  ...
  static final ComparisonOption previousPeriod = ComparisonOption(
    _prevPeriodMsg,
    (DatepickerDateRange? range) {
      var prev = range?.prev;
  ...
  static final ComparisonOption samePeriodLastYear = ComparisonOption(
    _previousYearMsg,
    (DatepickerDateRange? range) => DatepickerDateRange(
      _previousYearMsg,
      range?.start?.add(years: -1),
      range?.end?.add(years: -1),
    ),
  );

  static final ComparisonOption custom = ComparisonOption(
    _customMsg,
    (DatepickerDateRange? range) => null,
  );

  static final String _prevPeriodMsg = Intl.message(
    'Previous period',
    name: '_prevPeriodMsg',
    meaning:
        'An option name, the date range before the selected date '
        'range',
    desc:
        'Setting to compare the selected date range with the previous '
        'period. E.g. if the selected range were May, this would be April'
        '.',
  );
  static final String _previousYearMsg = Intl.message(
    'Previous year',
    name: '_previousYearMsg',
    meaning: 'An option name, the same date range in the last year',
    desc:
        'Setting to compare the selected date range with the same range '
        'last year. E.g. if the selected range were May 2015, this would'
        ' be May 2014.',
  );

  static final String _customMsg = Intl.message(
    'Custom',
    name: '_customMsg',
    meaning: 'An option name, user could enter the date range they want',
    desc:
        'Setting to compare the selected date range with another '
