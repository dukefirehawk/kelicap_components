...
class MaterialSliderComponent implements AfterChanges, HasDisabled {
  ...
  void ngAfterChanges() {
    assert(() {
      ...
      checkArgument(value >= min, message: 'Failed assertion: $value >= $min');
      checkArgument(value <= max, message: 'Failed assertion: $value <= $max');
      checkArgument(
  ...
  void mouseDown(MouseEvent event) {
    ...
    EventStreamProviders.mouseUpEvent.forTarget(document).take(1).listen((
      event,
    ) {
      event.preventDefault();
  ...
  void touchStart(TouchEvent event) {
    ...
    EventStreamProviders.touchEndEvent.forTarget(document).take(1).listen((
      event,
    ) {
      event.preventDefault();
