  ...
  void _initIfRequired() {
    if (_target == null) {
      ...
      if (_source is ObservableList) {
        ...
        _subscription = (_source as ObservableList).listChanges.listen(
          (event) => _onSourceChanges(event),
        );
      }
      _target!.length = _source.length;
    }
  ...
  void _onSourceChanges(List<ListChangeRecord> event) {
    ...
    for (int eventIndex = 0; eventIndex < event.length; eventIndex++) {
      ...
      if (record.removed.isNotEmpty) {
        for (
          int removedIndex = record.removed.length - 1;
          removedIndex >= 0;
          removedIndex--
        ) {
        ...
        _target!.removeRange(
          record.index,
          record.index + record.removed.length,
        );
      }
      ...
      if (record.addedCount > 0) {
        Iterable added = record.object.sublist(
          record.index,
          record.index + record.addedCount,
        );
