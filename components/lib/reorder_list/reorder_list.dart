...
import 'package:kelicap_components/reorder_list/reorder_events.dart';
import 'package:kelicap_components/utils/disposer/disposer.dart';
import 'package:kelicap_components/utils/keyboard/keyboard.dart';
...
class ReorderListComponent implements OnDestroy {
  ...
  final _itemSelectionChanged = StreamController<ItemSelectionEvent>.broadcast(
    sync: true,
  );
  ...
  void _moveItem(int fromIndex, int toIndex) {
    ...
    if (verticalItems) {
      final rect = draggedElement!.getBoundingClientRect();
      placeholder?.style
      ...
      final offsetRight = e.offsetLeft + e.offsetWidth;
      final rect = draggedElement!.getBoundingClientRect();
      var left = moveRight ? e.offsetLeft : offsetRight - rect.width;
  ...
  int? _horizontalTransformHandler(
    ...
    final offsetRight = e.offsetLeft + e.offsetWidth;
    if (offsetRight + offset > rightSide) {
      // Right side overflow
      nextOffset = e.offsetWidth as int?;
      e.style.transform =
          'translate(${-e.offsetLeft + leftSide!}px,'
          '${e.offsetHeight}px)';
    } else if (e.offsetLeft + offset < leftSide!) {
      // Left side overflow
      final prevOffsetRight = prev!.offsetLeft + prev.offsetWidth;
      if (e.offsetWidth < prevOffsetRight) {
        // Enough room in last row
        nextOffset = -e.offsetWidth as int?;
        e.style.transform =
            'translate('
            '${prevOffsetRight - leftSide + offset}px,'
            '${-e.offsetHeight}px)';
      } else {
        // No room in last row
        nextOffset = 0;
  ...
  void track(HTMLElement element) {
    ...
    subs.add(
      element.onDragStart.listen((e) {
        if (e.isA<DragEvent>()) {
          final dragEvent = e as DragEvent;
    ...
    _dragSubscriptions[element] = element.onDragOver.listen(
      (e) => _onDragOver(e, element),
    );
  ...
  void _onDragStart(MouseEvent e) {
    ...
    _itemSizes = List<int>.filled(childCount, 0, growable: false);
    for (int i = 0; i < childCount; i++) {
  ...
  int _computeItemSize(Element elem) {
    final cssStyle = window.getComputedStyle(elem);
    if (verticalItems) {
      return (elem as HTMLElement).offsetHeight +
          _parsePx(cssStyle.marginBottom) +
          _parsePx(cssStyle.marginTop);
    } else {
      return (elem as HTMLElement).offsetWidth +
  ...
  void _onDragOver(MouseEvent event, HTMLElement element) {
    ...
    if (_currentMoveIndex != moveTargetIndex &&
      ...
      _reorderProgress.add(
        _createReorderEvent(_moveSourceIndex, _currentMoveIndex),
      );
...
@Directive(selector: '[reorderHandle]', exportAs: 'handle')
