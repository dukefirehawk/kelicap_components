...
import 'package:kelicap_components/material_datepicker/comparison.dart';
...
import 'package:kelicap_components/src/material_datepicker/sequential.dart';
...
class DateRangeEditorModel
  ...
  final range = ObservableReference<DateRange?>(
    DateRange(null, null),
    coalesce: true,
  );

  @override
  final comparison = ObservableReference<DateRange>(
    DateRange(null, null),
    coalesce: true,
  );
  ...
  void _setEverything(DatepickerComparison? newValue) {
    ...
    final hypotheticalComparison = (comparison != null)
        ? comparison
        : _buildComparison(range).comparison;
    if (hypotheticalComparison == null) return;
    this.comparison.value = hypotheticalComparison.asPlainRange();
    _customComparisonRange = DatepickerDateRange.custom(
      hypotheticalComparison.start,
      hypotheticalComparison.end,
    );
  ...
  void _onCalendarChange(Change<CalendarState?> change) {
    ...
    if (newState.currentSelection == rangeId &&
      ...
      selectedRange = DatepickerDateRange.custom(
        newState.selection(rangeId).start,
        newState.selection(rangeId).end,
      );
  ...
  DatepickerComparison _buildComparison(DatepickerDateRange? range) {
    ...
    // their chosen range.
    final defaultCustomComparisonRange = DatepickerDateRange.custom(
      range!.start,
      range.start,
    );
    ...
    if (_comparisonOption == ComparisonOption.custom) {
      return DatepickerComparison.custom(
        range,
        _customComparisonRange ?? defaultCustomComparisonRange,
      );
    }
