...
class MaterialExpansionPanelSet implements OnDestroy {
    ...
    for (final panel in _panels) {
      ...
      _panelDisposer
        ..addStreamSubscription(
          (panel.isExpandedChange.listen((isExpanded) {
            if (isExpanded!) _setOpenPanel(panel);
          })),
        )
        ..addStreamSubscription(
          panel.open.listen((action) {
            _onPanelOpen(panel, action);
          }),
        )
        ..addStreamSubscription(
          panel.close.listen((action) {
            _onPanelClose(panel, action!);
          }),
        )
        ..addStreamSubscription(
          panel.cancel.listen((action) {
            _onPanelClose(panel, action!);
          }),
        );

      if (panel.closeOnSave) {
        _panelDisposer.addStreamSubscription(
          panel.save.listen((action) {
            _onPanelClose(panel, action!);
          }),
        );
      }
  ...
  void _onPanelOpen(
    ...
    action!.cancelIf(
      _openPanel!.collapse(byUserAction: false).then((success) {
